...
- **Pfad A Technische Hygiene, CI-Workflow-Härtung, Lock- & Cache-Schutz und Level 1 SBOM Re-Audit:**
  ...
  - **Packaging & PEP 621 Tooling in `pyproject.toml`:** `[project.urls]` um kanonischen `"Notice"`-Link ergänzt; Pytest-Konfiguration um `norecursedirs` gehärtet; Versionsnummer `0.4.0` strikt unverändert eingefroren (T-20260920-167562623).
  - **Level 1 SBOM Re-Audit (`THIRD_PARTY_LICENSES.md`):** Audit-Stand 2026-09-23 verifiziert; 100% permissiv, Zero-Copyleft-Isolationsgarantie, Null externe Core-Laufzeitabhängigkeiten (`dependencies = []`), Unprivileged User-Mode (`RunAsInvoker`) und 10-Punkte Invarianten-Matrix (`INV-LOCAL-01` bis `INV-SLA-10`) bestätigt.
  - **Dokumentations- & Kontextparität:** Shields.io-Testabzeichen in `README.md` und `README_de.md` auf 148 passed | 43 subtests synchronisiert; `llms.txt` und `MARKETING-LOG.txt` auf Stand 2026-09-23 aktualisiert.
  - **Metadaten- & CI-Vertragstestsuite (`tests/test_metadata.py`):** Neue Vertragstests für Welcome-Workflow, Stale-Concurrency, PEP 621 Notice-URL und kanonische Lock-Muster integriert (148 passed, 43 subtests | 100% grün) [G 2026-09-23].
